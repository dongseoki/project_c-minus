...
struct symbol *find_symbol(struct table *tablep, char *name)
{
	...
	for (symbolp = tablep->hash[hash_function(name)];
		symbolp != NULL; symbolp = symbolp->next)
...
void print_table(struct table *tablep)
{
	...
	for (i = 0; i < HASH_SIZE; i++)
		...
		{
			fprintf(stdout, "%-10s %-10s %-10s %-10s %10u %10u\n", tablep->name, symbolp->name, kinds[symbolp->kind], types[symbolp->type], symbolp->size, symbolp->offset );
			//fprintf(stdout, "test.symbolp->kind: %c, symbolp->type: %c\n", symbolp->kind, symbolp->type); // for test
		}
}
